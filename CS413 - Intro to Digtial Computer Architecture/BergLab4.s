@ Filename: BergLab4b.s
@ Author:   Eli Berg
@ Email:    erb0023@uah.edu
@           CS413-02 2025
@
@ Purpose:  Simulates the operation of a Keurig-style coffee machine but in ARM THUMB-1 Instruction Set
@
@ History:
@    Date       Purpose of change
@    ----       -----------------
@   18-March-2025  Student created outline for code 
@   19-March-2025  Student implemented code and test
@   21-March-2025  Student bug fixed code and finished assigment
@
@ Use these commands to assemble, link, run and debug this program:
@    as -o BergLab4.o BergLab4.s
@    gcc -o BergLab4 BergLab4.o
@    ./BergLab4 ;echo $?
@    gdb --args ./BergLab4 
...
main:
@*******************
    adr r0, prompt + 1    @ Generate address for Thunb-1
    bx r0                 @ Branch to Thumb-1 state
    .thumb                @ Assemble Thumb-1

@*******************
prompt:
@*******************
    mov r4, #48               @ Initialize water amount
    mov r5, #0                @ User inputs and cmp
    mov r6, #0                @ Initialize small cup count to 0
    mov r7, #0                @ Initialize medium cup count to 0
    ldr r3, =largeCupCount    @ Initialize large cup count to 0
    str r5, [r3]
...
@*******************
    ...
    cmp r5, #'B'            @ Check if input is 'B' (start brewing)
    beq select_cup
    cmp r5, #'b'            @ Check for lowercase 'b'
    beq select_cup
    cmp r5, #'T'            @ Check if input is 'T' (turn off)
    beq done
    cmp r5, #'t'            @ Check for lowercase 't'
    beq done
    cmp r5, #'H'            @ Check if input is 'H' (hidden inventory)
    beq hidden_inv
    cmp r5, #'h'            @ Check for lowercase 'h'
    beq hidden_inv
...
@*******************
    ...
    cmp r5, #1              @ If input is 1, set small cup size
    beq set_small
    cmp r5, #2              @ If input is 2, set medium cup size
    beq set_medium
    cmp r5, #3              @ If input is 3, set large cup size
    beq set_large
...
set_small:
@*******************
    mov r5, #6       @ Set cup size to 6 oz
    b check_water

@*******************
set_medium:
@*******************
    mov r5, #8       @ Set cup size to 8 oz
    b check_water

@*******************
set_large:
@*******************
    mov r5, #10      @ Set cup size to 10 oz
    b check_water

@*******************
check_water:
@*******************
    cmp r5, r2       @ Compare water level with cup size
...
brew:
@*******************
    sub r4, r4, r5    @ Subtract selected cup size from water level
...
update_large:
@*******************
    ldr r3, =largeCupCount
    ldr r1, [r3]
    add r1, r1, #1  @ Increment large cup count
    str r1, [r3]   @ Store updated large cup count in memory
    b post_brew
...
@*******************
    ...
    cmp r4, #6                @ Check if water is too low
...
.balign 4
smallCups:      .asciz "Small cups dispensed: %d\n"
.balign 4
mediumCups:     .asciz "Medium cups dispensed: %d\n"
.balign 4
largeCups:      .asciz "Large cups dispensed: %d\n"
